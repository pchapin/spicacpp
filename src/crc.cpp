/*! \file   crc.cpp
 *  \brief  CRC checksum calculating functions.
 *  \author Peter Chapin <spicacality@kelseymountain.org>
 */

#include <spicacpp/crc.hpp>
...
unsigned short CRC_Update( unsigned short CRC, unsigned char CRC_Char )
{
    ...
    /* The variable "x" will contain the character to be processed in bits 0-7 and the CRC in
       bits 8-23. Bit 24 will be used to test for overflow, and then cleared to prevent the sign
       bit of "x" from being set to 1. Bits 25-31 are not used. ("x" is treated as though it is
       a 32 bit register). */
    ...
    for( i = 0; i < 8; i++ ) {
        ...
        if( x & 0x01000000UL )
            ...
            /* If the old high-order bit of the CRC was a 1, exclusive-or it with a one to set
               ...
               CRC generator of: X**16 + X**12 + X**5 + 1. To produce the CRC generator of:
               X**16 + X**15 + X**2 + 1, change the constant from 0x01102100 to 0x01800500. This
               will exclusive-or the CRC with hex 8005 and produce the same CRC that IBM uses
               for their synchronous transmission protocols. */
