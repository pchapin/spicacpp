...
#include <spicacpp/synchronize.hpp>
#endif
...
namespace spica {
    ...
    {
        std::unique_ptr<string_node> new_node = std::make_unique<string_node>();
    ...
    {
        unique_ptr<string_node> new_node = std::make_unique<string_node>();
     ...
     * \param pad The pad character to use if the length is too large.
    ...
    RexxString RexxString::right(size_t length, char pad) const
    {
        ...
        // If we need to shorten the string...
     ...
     * unchanged.
     */
    RexxString RexxString::insert(const RexxString& incoming, size_t starting_position,
                                  size_t count) const
    {
#if defined(pMULTITHREADED)
        mutex_sem::grabber lock(string_lock);
        ...
        if (count > incoming_length)
            count = incoming_length;
    ...
    RexxString RexxString::substr(size_t starting_position, size_t count) const
    {
        ...
        if (count > current_length - offset)
            count = current_length - offset;
    ...
    RexxString RexxString::subword(size_t starting_position, size_t count, const char* white) const
    {
        ...
        while (true) {
            ...
            // Find the end of this word.
            while (!(is_white(*end, white) || *end == '\0'))
                end++;
            count--;
