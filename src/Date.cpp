...
namespace spica {
    ...
    void Date::next()
    {
        // Try incrementing the day. If it overflows the month, make a correction.
        if (++D > month_length()) {
            D = 1;
            if (++M > 12) {
                M = 1;
    ...
    void Date::set(int year, int month, int day)
    {
        ...
        if (Y > 2099)
            Y = 2099;
        if (M < 1)
            M = 1; //! \todo Deal with out of bounds values by adjusting entire date.
        if (M > 12)
            M = 12;
        if (D < 1)
            D = 1;
