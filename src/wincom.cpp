...
namespace spica {
    namespace Windows {
        ...
        namespace {
            ...
            void display_DCB(const char* header, const DCB& the_DCB)
            {
                ...
                message << "Baud=" << the_DCB.BaudRate << ' '
                        ...
                        << "DataBits=" << the_DCB.ByteSize << ' ' << "StopBits=";
            ...
            void display_COMMTIMEOUTS(const char* header, const COMMTIMEOUTS& the_TIMEOUTS)
            {
                ...
                message << header << ":";
                message << " ReadIntervalTimeout=" << the_TIMEOUTS.ReadIntervalTimeout
                        << " WriteTotalTimeoutConstant=" << the_TIMEOUTS.WriteTotalTimeoutConstant
                ...
                try {
                    ...
                    if (the_port->testing_mode) {
                        ...
                        // If no problem, wait for the result.
                            ...
                            GetOverlappedResult(the_port->the_handle, &overlapped, &byte_count,
                                                TRUE);
                            if (event != EV_RXCHAR)
                                continue;
                        }
                        ...
                        read_result = ReadFile(the_port->the_handle, the_bytes, BUFFER_SIZE,
                                               &byte_count, &overlapped);
                    ...
                    message << "Out of memory while processing input from port " << the_port->name;
                    message.say();
                }
                catch (APIError We) {
                    notifystream message;
        ...
        ComPort::ComPort(bool testing) :
            name(0), port_set(false), outer_usage(false), testing_mode(testing), read_processor(0),
            helperThread_handle(INVALID_HANDLE_VALUE), input_buffer(128)
        {
        ...
        void ComPort::set(const char* given_name, int baud, void (*read)(char))
        {
            ...
            // Can I open the port?
            the_handle = CreateFile(given_name,                   // Name of port.
        ...
        void ComPort::write(const char* outgoing)
        {
            ...
            write_result =
                WriteFile(the_handle, outgoing, string_length, &bytes_written, &overlapped);
