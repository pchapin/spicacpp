...
namespace spica {
    ...
    template <class Callable> struct Parameters0 {
        const Callable f;

        Parameters0(Callable initial_f) : f(initial_f)
        {
        }
    };

    template <class Callable, class Parameter1Type> struct Parameters1 {
        const Callable f;
    ...
    template <class Callable, class Parameter1Type, class Parameter2Type> struct Parameters2 {
        const Callable f;
        Parameter1Type param1;
        Parameter2Type param2;

        Parameters2(Callable initial_f, Parameter1Type initial_param1,
                    Parameter2Type initial_param2) :
    ...
    template <class Callable> void thread_launcher0(void* raw_parameters)
    {
    ...
    template <class Callable, class Parameter1Type> void thread_launcher1(void* raw_parameters)
    {
    ...
    class ThreadPool {
      private:
        ...
        struct ThreadInformation {
            ...
            void (*launcher)(
                void*); // Pointer to helper function that launches thread into its callable.
      ...
      public:
        ...
        template <typename Callable> threadid_t start_work(Callable work_function);

        template <typename Callable, typename Parameter1Type>
        threadid_t start_work(Callable work_function, const Parameter1Type& parameter_1);
                                                  ...
                                                  const Parameter2Type& param_2)
    {
        ...
        for (; info != thread_information + pool_size; ++info) {
            ...
            if (info->fresh_work == false) {
                ...
                // Set up the parameters structure.
                auto raw_parameters = new Parameters2<Callable, Parameter1Type, Parameter2Type>(
                    work_function, param_1, param_2);

                // Initialize the thread information structure.
                info->launcher = thread_launcher2<Callable, Parameter1Type, Parameter2Type>;
                info->raw_parameters = raw_parameters;
                info->fresh_work = true;
                info->fresh_result = false;
