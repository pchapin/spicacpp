...
namespace spica {
      ...
      public:
        ...
        class iterator {
          ...
          private:
            ...
            iterator(SingleList* o, Node* p, Node* c) noexcept : object(o), previous(p), current(c)
            {
            }

          public:
            //! Default constructor creates a NULL iterator.
            iterator() noexcept : object(nullptr), previous(nullptr), current(nullptr)
            {
            }
    ...
    template <typename T> SingleList<T>::~SingleList() noexcept
    {
    ...
    template <typename T> SingleList<T>::SingleList(const std::initializer_list<T>& initializers)
    {
    ...
    template <typename T> SingleList<T>::SingleList(const SingleList& other)
    {
    ...
    template <typename T> SingleList<T>& SingleList<T>::operator=(const SingleList& other)
    {
        if (this != &other) {
    ...
    SingleList<T>::SingleList(SingleList&& other) noexcept :
        head_node(other.head_node), last_node(other.last_node), count(other.count)
    {
    ...
    template <typename T> SingleList<T>& SingleList<T>::operator=(SingleList&& other) noexcept
    {
        if (this != &other) {
    ...
    template <typename T>
    template <typename InputIterator>
    typename SingleList<T>::iterator SingleList<T>::insert(iterator& p, InputIterator first,
                                                           InputIterator last)
    {
        iterator result(p);
    ...
    template <typename T> typename SingleList<T>::iterator SingleList<T>::begin() noexcept
    {
        return iterator(this, nullptr, head_node);
    }

    template <typename T> typename SingleList<T>::iterator SingleList<T>::end() noexcept
    {
