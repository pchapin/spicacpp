...
namespace spica {
    ...
    template <POD T> class FileVector {
      public:
    ...
    template <typename T> bool operator==(const FileVector<T>&, const FileVector<T>&);
...
namespace spica {
    ...
    //
    template <POD T> void FileVector<T>::reserve(size_type new_capacity)
    {
    ...
    //
    template <POD T> void FileVector<T>::push_back(const T& new_item)
    {
        reallocate(1);
    ...
    //
    template <POD T>
    typename FileVector<T>::iterator FileVector<T>::insert(iterator position, const T& new_item)
    {
        reallocate(1);
    ...
    //
    template <POD T> void FileVector<T>::insert(iterator position, size_type n, const T& fill)
    {
        reallocate(n);
    ...
    //
    template <POD T> void FileVector<T>::erase(iterator position)
    {
        std::memmove(position, position + 1, sizeof(T) * ((raw + item_count) - (position + 1)));
        item_count--;
    }
    ...
    //
    template <POD T> void FileVector<T>::erase(iterator first, iterator last)
    {
    ...
    //
    template <POD T> FileVector<T>::FileVector(const char* file_name)
    {
        // Open the file.
        file_handle = CreateFile(file_name,                    // The name (of course).
        ...
        // Map it.
        mapping_handle = CreateFileMapping(file_handle,    // The file we are trying to map.
                                           0,              // Default security attributes.
        ...
        raw = static_cast<T*>(
            MapViewOfFile(mapping_handle,      // The mapped file from which we create the view.
    ...
    // The constructor opens and maps the file.
    //
    template <POD T> FileVector<T>::FileVector(const char* file_name, size_type n, const T& initial)
    {
        // Open the file.
        file_handle = CreateFile(file_name,                    // The name (of course).
                                 GENERIC_READ | GENERIC_WRITE, // I want to read and write the file.
    ...
    //
    template <POD T> FileVector<T>::~FileVector()
    {
        UnmapViewOfFile(raw);
        CloseHandle(mapping_handle);
    ...
    //
    template <POD T> void FileVector<T>::swap(FileVector<T>& other)
    {
        using std::swap;
    ...
    //
    template <POD T> void FileVector<T>::reallocate(size_type more)
    {
        ...
        if (item_count + more <= item_capacity)
            return;
