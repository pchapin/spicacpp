...
namespace spica {
    ...
    class RexxString {
      ...
      public:
        ...
        [[nodiscard]] RexxString
        erase(std::size_t starting_position,
              std::size_t count = std::numeric_limits<std::size_t>::max()) const;

        //! Insert a RexxString into this RexxString.
        [[nodiscard]] RexxString
        insert(const RexxString& incoming, std::size_t starting_position = 1,
        ...
        [[nodiscard]] RexxString
        substr(std::size_t starting_position,
               std::size_t count = std::numeric_limits<std::size_t>::max()) const;

        //! Locate a substring of this RexxString consisting of the specified number of words.
        RexxString subword(std::size_t starting_position,
