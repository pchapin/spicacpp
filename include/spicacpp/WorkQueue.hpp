...
namespace spica {
    ...
    //
    // This function initializes the synchronization primitives needed to make this work. Do not
    // attempt to use a WorkQueue object until the thread calling its constructor has returned.
    //
    template <typename T>
    inline WorkQueue<T>::WorkQueue(int max_size) : free_slots(max_size), used_slots(0)
    {
    }
