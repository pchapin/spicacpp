...
void check_assignment()
{
    ...
    target = source_1;
    UNIT_CHECK(target == source_1);
    target = source_2;
    UNIT_CHECK(target == source_2);
    target = source_3;
    UNIT_CHECK(target == source_3);
}
...
void check_bit_manipulation()
{
    ...
    UNIT_CHECK(object_1.get_bit(0) == 0 && object_1.get_bit(1) == 0 && object_1.get_bit(33) == 0);
    UNIT_CHECK(object_2.get_bit(0) == 1 && object_2.get_bit(1) == 0 && object_2.get_bit(2) == 1 &&
               object_2.get_bit(3) == 0 && object_2.get_bit(33) == 0);

    object_1.put_bit(0, 1);
    object_1.put_bit(33, 1);

    UNIT_CHECK(object_1.get_bit(0) == 1 && object_1.get_bit(1) == 0 && object_1.get_bit(33) == 1);
...
void check_plus()
{
    ...
    target = zero;
    target += zero;
    UNIT_CHECK(target == zero);
    target = zero;
    target += object_1;
    UNIT_CHECK(target == object_1);
    target = object_1;
    target += zero;
    UNIT_CHECK(target == object_1);
    target = object_1;
    target += object_1;
    UNIT_CHECK(target == result_1);
    target = object_2;
    target += one;
    UNIT_CHECK(target == result_2);
    target = object_1;
    target += object_3;
    UNIT_CHECK(target == zero);
    target = object_1;
    target += minus_one;
    UNIT_CHECK(target == result_3);
    target = minus_one;
    target += object_1;
    UNIT_CHECK(target == result_3);
    target = object_3;
    target += minus_one;
    UNIT_CHECK(target == result_4);
    target = object_4;
    target += one;
    UNIT_CHECK(target == result_5);
    target = object_5;
    target += one;
    UNIT_CHECK(target == result_6);
}
...
void check_minus()
{
    ...
    target = object_4;
    target -= one;
    UNIT_CHECK(target == result_7);
    target = object_5;
    target += one;
    UNIT_CHECK(target == result_8);
}
